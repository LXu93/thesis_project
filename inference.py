...
def inference_diffusion(prompt, 
    ...
    with torch.no_grad():
        ...
        for i in pbar:
            if pathology:
                prompt = prompt_generator(pathology)
            if random_gs:
...
if __name__ == "__main__":
    inference_diffusion(image_nums = 10,
                        prompt="endoscopy image showing a bubble",
                        # pathology="polyp", 
                        num_inference_steps= 100,
                        #guidance_scale = 8,
                        random_gs=(7,15),
                        trained_model_dir="checkpoints/bubbles_and_dirt/epoch_9",
                        output_dir="output/bubbles_and_dirt/bubbles/epoch_9")
